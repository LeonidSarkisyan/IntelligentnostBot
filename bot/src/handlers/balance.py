...
from src.keyboards.balance import get_balance_show_keyboard
...
async def show_balance(call: CallbackQuery):
    ...
    await call.message.edit_text(get_balance_show_message(user.balance))


@router.message(F.text == "/balance")
async def show_balance(message: Message):
    user = await user_service.get(message.from_user.id)
    await message.answer(get_balance_show_message(user.balance))
